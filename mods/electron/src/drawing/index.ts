...
export function drawTile(uiCtx: UICtx, x: number, y: number, color: string) {
  const xp = x * uiCtx.tilePx - uiCtx.getCameraPosition().x
  const yp = uiCtx.canvasHeightPx - (y + 1) * uiCtx.tilePx - uiCtx.getCameraPosition().y
...
export function drawText(uiCtx: UICtx, x: number, y: number, color: string, text: string) {
  ...
  const xp = x * uiCtx.tilePx + uiCtx.tilePx / 2 - uiCtx.getCameraPosition().x
  const yp = uiCtx.canvasHeightPx - (y + 1) * uiCtx.tilePx + uiCtx.tilePx / 2 + verticalOffsetPx - uiCtx.getCameraPosition().y
